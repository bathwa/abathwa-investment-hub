...
CREATE POLICY "work_orders_participants_view" ON public.work_orders
  FOR SELECT TO authenticated USING (
    ...
    EXISTS (SELECT 1 FROM service_requests WHERE id = work_orders.service_request_id AND requester_id = auth.uid())
  );
...
CREATE POLICY "job_cards_requestor_view" ON public.job_cards
  FOR SELECT TO authenticated USING (
    EXISTS (
      ...
      WHERE wo.id = job_cards.work_order_id AND sr.requester_id = auth.uid()
    )
  );
...
CREATE POLICY "service_negotiations_participants_manage" ON public.service_negotiations
  FOR ALL TO authenticated USING (
    service_provider_id = auth.uid() OR 
    EXISTS (SELECT 1 FROM service_requests WHERE id = service_negotiations.service_request_id AND requester_id = auth.uid())
  );
